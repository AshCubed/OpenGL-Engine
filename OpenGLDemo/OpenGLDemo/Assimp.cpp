...
bool ModelManager::assimpGetMeshData(const aiMesh* mesh)
{
    ...
    vertexBuff.resize(indexBuff.size(), Vertex());

    for (unsigned int v = 0; v < mesh->mNumVertices; v++)
    {
        vertexBuff[v].position = glm::vec3(mesh->mVertices[v].x, mesh->mVertices[v].y, mesh->mVertices[v].z);
        vertexBuff[v].normal = glm::vec3(mesh->mNormals[v].x, mesh->mNormals[v].y, mesh->mNormals[v].z);

        if (mesh->HasTextureCoords(0)) {
            vertexBuff[v].texcoord = glm::vec2(mesh->mTextureCoords[0][v].x, mesh->mTextureCoords[0][v].y);
        }
        else
        {
            vertexBuff[v].texcoord = glm::vec2(0, 0);
        }

        vertexBuff[v].color = glm::vec3(1.f, 1.f, 1.f);
...
std::vector<Vertex>* ModelManager::getVertexData()
{
    return &vertexBuff;
}

std::vector<GLint>* ModelManager::getIndexData()
{
    return &indexBuff;
}
